-- FTAP V14R4 FREEZE LAB - ONE-TIME STUDIO INSTALLER
-- Run this entire file in Roblox Studio's Command Bar while editing YOUR game.
-- It creates/updates ServerScriptService.FTAPFreezeLab with the bundled server lab.

local ServerScriptService = game:GetService("ServerScriptService")
local SCRIPT_NAME = "FTAPFreezeLab"
local SOURCE = [==[
...
local existing = ServerScriptService:FindFirstChild(SCRIPT_NAME)
if existing and not existing:IsA("Script") then
    error("ServerScriptService."..SCRIPT_NAME.." exists but is not a Script")
end
...
print("[FTAP Freeze Lab] installed/updated ServerScriptService."..SCRIPT_NAME)
print("[FTAP Freeze Lab] now use Test > Start Server + Players, or publish and use a private server.")
